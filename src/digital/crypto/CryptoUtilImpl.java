...
public class CryptoUtilImpl {
    ...
    public String encryptAES(byte[] data, SecretKey secretKey) throws Exception {
        ...
        public String encryptAESToHex(byte[] data, SecretKey secretKey) throws Exception {
        Cipher cipher = Cipher.getInstance("AES");
        cipher.init(Cipher.ENCRYPT_MODE, secretKey);
        byte[] encryptedData = cipher.doFinal(data);
        return encodeToHexNative(encryptedData);
    }
...
public byte[] decryptAESFromHex(String hexData, SecretKey secretKey) throws Exception {
    byte[] encryptedData = decodeFromHex(hexData);
    Cipher cipher = Cipher.getInstance("AES");
    cipher.init(Cipher.DECRYPT_MODE, secretKey);
    return cipher.doFinal(encryptedData);
}

private byte[] decodeFromHex(String hexData) {
